...
private fun LibrarySeriesCard(
    ...
    Surface(
        modifier = modifier
            .fillMaxWidth()
            .clickable { onOpenOfflineSeries(group) },
        shape = RoundedCornerShape(10.dp),
        tonalElevation = 0.dp,
    ) {
        Column(
...
private fun LibrarySeriesDetailScreen(
    ...
    val seriesEpisodes = group.episodes.sortedWith(
        compareBy<OfflineDownload> { it.seasonNumber ?: Int.MAX_VALUE }
            .thenBy { it.episodeNumber ?: Int.MAX_VALUE }
            .thenBy { it.contentId },
    )
    val episodeFocusRequester = remember { FocusRequester() }
    val targetEpisodeIndex = seriesEpisodes.indexOfFirst { it.contentId == returnFocusEpisodeId }

    LaunchedEffect(returnFocusGeneration, targetEpisodeIndex) {
        if (returnFocusGeneration <= 0 || targetEpisodeIndex < 0) return@LaunchedEffect
        withFrameNanos { }
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = requireNotNull(
                        librarySeriesOfflineLabel(
                            group.episodes.count(OfflineDownload::countsForOfflineFilter),
                        ),
                    ),
                    style = MaterialTheme.typography.labelMedium,
                    color = MaterialTheme.colorScheme.primary,
            ...
            onOpenFullSeries?.let { open ->
                TextButton(onClick = open) { Text("Open full series") }
            }
        }
        ...
        LazyColumn(
            ...
            listItems(
                ...
                LibraryEpisodeRow(
                    ...
                    onRetry = onRetry,
                    onRemove = onRemove,
                    modifier = if (download.contentId == returnFocusEpisodeId) {
    ...
    Surface(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = buildString {
                        download.seasonNumber?.let { append("S$it ") }
                        download.episodeNumber?.let { append("E$it · ") }
...
@Composable
private fun LibraryOfflineBadge() {
    Surface(
        ...
        Row(
            ...
            Icon(Icons.Filled.DownloadDone, contentDescription = null, modifier = Modifier.size(14.dp))
            Text("Offline", style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Bold)
        }
    }
}
